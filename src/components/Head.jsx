
import { Link, Outlet } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { tooglepopbar } from "../utils/toogleSlice";


function Head() {
  
  const cartData = useSelector((state)=> state.cartSlice.cartItems)
  const dispatch = useDispatch();
  
  const open = useSelector((state) => state.toogleSlice.searchToogle);


  function handleArrowGo() {
    dispatch(tooglepopbar()); 
  }

  const navItems = [
    { name: "Search", image: <i className="fi fi-rr-search text-2xs"></i>, path: "/search" },
    { name: "Offers", image: <i className="fi fi-rr-shopping-bag text-2xs"></i>, path: "/offer" },
    { name: "Help", image: <i className="fi fi-sr-life-ring text-2xs "></i>, path: "/help" },
    { name: "Sign In", image: <i className="fi fi-br-sign-in-alt text-2xs"></i>, path: "/signin" },
    { name: "Cart", image: <i className="fi fi-ss-shopping-cart text-2xs"></i>, path: "/cart" },
  ];

  return (
    <>
      {open && (
        <div className="w-full z-10 h-[100vh] absolute bg-black/40">
          <div className="w-[40%] h-full bg-white overflow-y-hidden">
            <div className="w-full h-full p-10 flex flex-col gap-7">
          
              <p className="flex text-black" onClick={handleArrowGo}>
                <i className="fi fi-rr-cross-small text-3xl"></i>
              </p>
              
              {/* Search bar */}
              <div className="h-14 w-[400px] border shadow-lg">
                <input
                  type="text"
                  placeholder="Search for area, street name.."
                  className="w-full p-4 border border-gray-400 shadow-md focus:outline-gray-100"
                />
              </div>

              {/* Location section */}
              <div className="h-20 w-[400px] flex border shadow-lg gap-3 p-2">
                <div className="flex">
                  <i className="fi fi-tr-location-crosshairs mt-1"></i>
                </div>
                <div className="flex-col hover:text-orange-500">
                  <p>Get current location</p>
                  <p className="text-xs text-gray-400">Using GPS</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      )}

      {/* Navbar */}
      <div className="h-16 w-full shadow-md flex items-center justify-center relative">
        <div className="flex w-[90%] justify-between">
          {/* Logo and dropdown */}
          <div className="w-1/4 h-14 flex items-center gap-4">
            <div className="flex items-center gap-7">
              <Link to="/">
                <img src="/image.png" alt="" className="h-11" />
              </Link>
              <p className="text-2xs text-cyan-800 border-b-2 font-bold border-b-neutral-800">Other</p>
            </div>
            <div className="mt-2" onClick={handleArrowGo}>
              <i className="fi fi-ss-angle-small-down text-xl text-orange-500"></i>
            </div>
          </div>

          {/* Navigation items */}
          <div className="w-3/5 h-14 flex justify-evenly gap-5 items-center">
            {navItems.map((data, index) => (
              <Link to={data.path} key={index}>
                <div className="flex gap-2">
                  <div className="mt-0 group">
                    <p className="text-gray-500 hover:text-orange-500">{data.image}</p>
                  </div>
                  <p className="font-bold text-gray-600 cursor-pointer hover:text-orange-500">
                    {data.name}
                  </p>
                  {data.name === "Cart" && cartData.length > 0 && (
                    <span className="text-white bg-orange-500 text-xs rounded-full px-2 py-0.5">
                      {cartData.length}
                    </span>
                  )}
                </div>
              </Link>
            ))}
          </div>
        </div>
      </div>

      <Outlet />
    </>
  );
}

export default Head;
